...
if prompt := st.chat_input("What's up?"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            with st.spinner("🤔 Thinking..."):
                ...
                with st.expander("🔍 Search and Reasoning Process", expanded=True):
                    ...
                    if hasattr(response, 'metadata') and response.metadata:
                        ...
                        if 'thought_process' in response.metadata:
                            st.markdown("**Step-by-Step Reasoning:**")
                            st.markdown(response.metadata['thought_process'])
                        elif 'agent_log' in response.metadata:
                            st.markdown("**Agent's Reasoning:**")
                            st.markdown(response.metadata['agent_log'])
                        elif 'prompt' in response.metadata:
                            st.markdown("**Analysis:**")
                            st.markdown(f"```\n{response.metadata['prompt']}\n```")
                
                # Show final response
                final_response = str(response).replace("<think>", "").strip()
    ...
    st.session_state.messages.append({
        "role": "assistant", 
        "content": str(response).replace("<think>", "").strip()
    })
